...
let ListController = class ListController {
    ...
    async fuzzySearch(dto) {
        if (!dto.keyword || dto.keyword.trim() === '') {
            const resp = response_util_1.ResponseUtil.error('搜索关键词不能为空', 400);
        ...
        if (dto.categoryId) {
            ...
            if (isNaN(categoryIdNum) || categoryIdNum <= 0) {
                const resp = response_util_1.ResponseUtil.error('无效的分类ID格式', 400);
            ...
            if (!validation.valid) {
                ...
                const resp = response_util_1.ResponseUtil.error(`${validation.message}。${availableMsg}`, 400);
    ...
    async clearFilterCache(channeid) {
        ...
        const resp = response_util_1.ResponseUtil.success(null, message);
